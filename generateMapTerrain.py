...
mcEditorLocation = "mcedit/pymclevel/"
sys.path.append(mcEditorLocation)
mcEditorLocation = "mcedit/"
sys.path.append(mcEditorLocation)
import mclevel
...
def main():
    ...
    print (sys.argv)
    worldLocation = sys.argv[1]
    world = mclevel.fromFile(worldLocation);
    # Image information
    ...
    infoElevation      = ElevationParse.getElevationInfo(sys.argv[2]);
    infoTerrain        = TerrainParse.getTerrainInfo(sys.argv[3]);
    infoDeposit        = DepositParse.getDepositInfo(sys.argv[4]);
    # Image selection
    #imageElevation      = Image.open(raw_input("Enter in Elevation Map: "));
    #imageTerrain        = Image.open(raw_input("Enter in Terrain Map: "));
    imageElevation      = Image.open(sys.argv[5]);
    imageTerrain        = Image.open(sys.argv[6]);
    # Transformation parameters
    imageOffset         = [int(sys.argv[7]),int(sys.argv[8])];
            ...
            for blockPosX in range(0, 16):
                for blockPosY in range(0, 16):
                    ...
                    if valid:
                        ...
                        chunk.Blocks[blockPosY,15-blockPosX,height-1:128] = BEDROCK;
                        chunk.Blocks[blockPosY,15-blockPosX,0:height] = STONE;
                    # Create Terrain 
                    if valid:
                        column = Column();
